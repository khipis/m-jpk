...
import static pl.softcredit.mpjk.engine.TestPaths.SHA_FILE_NAME_SCHEME_VERSION_1;
import static pl.softcredit.mpjk.engine.TestPaths.SHA_FILE_NAME_SCHEME_VERSION_2;
...
import static pl.softcredit.mpjk.engine.processors.JpkProcessors.SHA256_GENERATOR_STAGE_PROCESSOR;

@RunWith(MockitoJUnitRunner.class)
public class ShaGeneratorStageProcessorTest {

    @Mock
    private JpkConfiguration config;

    private JpkProcessor shaGeneratorStageProcessor = SHA256_GENERATOR_STAGE_PROCESSOR;
    ...
    @Test
    public void shouldSaveFileWithGeneratedSha256IntoWorkingDirectoryForVersion1() throws Exception {
        whenConfigurationWith(XML_FILE_NAME_SCHEME_VERSION_1, JPK_VAT_SCHEME_FILE_NAME_VERSION_1);

        shaGeneratorStageProcessor.process(config);

        assertFile(SHA_FILE_NAME_SCHEME_VERSION_1);
    }

    @Test
    public void shouldSaveFileWithGeneratedSha256IntoWorkingDirectoryForVersion2() throws Exception {
        whenConfigurationWith(XML_FILE_NAME_SCHEME_VERSION_2, JPK_VAT_SCHEME_FILE_NAME_VERSION_2);

        shaGeneratorStageProcessor.process(config);

        assertFile(SHA_FILE_NAME_SCHEME_VERSION_2);
    }
